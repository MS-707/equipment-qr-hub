...
function makeReq(body: unknown): Request {
  return new Request('http://localhost/api/safety/suggest-jha', {
    method: 'POST',
...
describe('POST /api/safety/suggest-jha', () => {
  ...
  it('returns 400 when no steps provided', async () => {
    process.env.NEXT_PUBLIC_AI_ASSIST = '1'
    process.env.ANTHROPIC_API_KEY = 'test-key'
    const { POST } = await import('@/app/api/safety/suggest-jha/route')
    const res = await POST(makeReq({ jobTitle: 'Test', steps: [] }))
    expect(res.status).toBe(400)
    const data = await res.json()
    expect(data.error).toContain('No task steps')
  })

  it('truncates steps to 15 max', async () => {
    process.env.NEXT_PUBLIC_AI_ASSIST = '1'
    process.env.ANTHROPIC_API_KEY = 'test-key'
