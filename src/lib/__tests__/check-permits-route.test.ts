...
function makeReq(body: unknown): Request {
  return new Request('http://localhost/api/safety/check-permits', {
    method: 'POST',
...
describe('POST /api/safety/check-permits', () => {
  it('returns 404 when AI assist is disabled', async () => {
    const { POST } = await import('@/app/api/safety/check-permits/route')
    const res = await POST(makeReq({ scopeOfWork: 'test' }))
    expect(res.status).toBe(404)
    const data = await res.json()
    expect(data.missing_permits).toEqual([])
  })
  ...
  it('returns 400 when scope of work is empty', async () => {
    process.env.NEXT_PUBLIC_AI_ASSIST = '1'
    ...
    const data = await res.json()
    expect(data.error).toContain('No scope of work')
  })
  ...
  it('returns 400 for invalid JSON body', async () => {
    ...
    process.env.ANTHROPIC_API_KEY = 'test-key'
    const { POST } = await import('@/app/api/safety/check-permits/route')
    const req = new Request('http://localhost/api/safety/check-permits', {
      method: 'POST',
      body: 'not json',
    })
