...
describe('buildSageContext', () => {
  ...
  it('extracts active SDS from page URL', async () => {
    vi.mocked(getSdsById).mockReturnValue({
      id: 'SDS-001',
      productName: 'Acetone',
      manufacturer: 'ChemCo',
      signalWord: 'Danger',
      ...
      firstAid: { inhalation: 'Move to fresh air', skin: 'Wash', eyes: 'Flush', ingestion: 'Do not induce vomiting' },
    } as never)
    const { buildSageContext } = await import('../sage-context')
    const ctx = buildSageContext('/sds/SDS-001', 'Alice')
    expect(ctx.activeChemicalContext).toContain('Acetone')
    expect(ctx.activeChemicalContext).toContain('67-64-1')
  })
})
