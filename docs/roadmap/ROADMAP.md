# Roadmap to 10/10 × 6 — frozen 2026-07-06
...
**22 milestones · 46 tasks · 30 criteria to flip · starting score 30/60**
...
- [ ] **LG-M1-T1** Rewrite the Digital Signatures and Resend sections of src/app/privacy/page.tsx to disclose that the pre-trip inspection signature is emailed to EHS as a PNG attachment via Resend (or, alternatively, stop attaching it in api/inspections/notify/route.ts).
...
- [ ] **LG-M1-T2** Reconcile the AI Features section with sage-context: either remove userName and supervisor name from contextToPrompt/summarizePtp in src/lib/sage-context.ts, or amend the privacy AI section to state worker names may be included in AI requests.
...
- [ ] **LG-M1-T3** Add Sentry (error monitoring), Upstash Redis / Vercel KV (server-side storage of login emails, beta signups, review submissions), and Vercel (hosting) to the Third-Party Services list, and extend the Slack bullet to cover first-sign-in name/email notifications.
...
- [ ] **EN-M4-T2** Enforce the ehs role server-side on at least one route: require role ehs-or-admin on api/safety/review/decide's session-fallback path (or add the check to review/status detail access), returning 403 for workers; keep the HMAC email-link path working. Add a route test for the 403.
...
- [ ] **EN-M2-T3** Write docs/RUNBOOKS.md with (a) numbered incident-response steps (detect via Sentry/health endpoint, contain by rotating NEXTAUTH_SECRET / revoking KV token / disabling env flags on Vercel, notify per SECURITY.md, post-mortem) and (b) backup/restore: add scripts/backup-kv.mjs that dumps review:*/beta:* keys to JSON via @upstash/redis SCAN, plus documented Notion export/restore steps and localStorage device-loss caveats.
...
### 17. EN-M1 — Dependency hygiene: triage and document supply-chain posture
...
### 18. LG-M3 — Third-party license notices artifact
...
### 19. DS-M1 — Tokenize every screen color (kill raw hex and default-palette classes)
...
### 20. DS-M2 — Type scale and shared button primitive
...
### 21. DS-M3 — Printable signed pre-trip inspection record
...
### 22. DS-M4 — Branded link previews (OG/Twitter)
...
## Beyond 10/10 — backlog (owner decision needed to promote)

The completeness critic found material workstreams outside the frozen rubrics. Two were folded
into milestones (impersonation hardening → EN-M4-TX1, CI enforcement → UX-M3-TX1); the rest
need a product/owner decision before the loop may touch them:
