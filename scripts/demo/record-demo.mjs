...
// The SDS library is a standalone app; the in-app "SDS Library" tile links out to it.
const SDS_URL = process.env.DEMO_SDS_URL || 'https://sds-five-beta.vercel.app/'
...
async function recordOrientation({ name, viewport, video }) {
  ...
  // ── Beat 2 — SDS: the "SDS Library" tile opens the standalone SDS app ───────
  await safe('sds', async () => {
    // Dwell on the dashboard quick actions, where the "SDS Library" tile lives.
    await page.goto(`${BASE_URL}/safety`, { waitUntil: 'networkidle' })
    await page.waitForTimeout(BEAT)
    // Tapping it opens the standalone SDS app (here we navigate directly so the
    // beat is recorded inline rather than in a popup tab).
    await page.goto(SDS_URL, { waitUntil: 'networkidle' })
    await page.waitForTimeout(BEAT)
    // Slow scroll to reveal the chemical library.
    for (const y of [300, 650]) {
