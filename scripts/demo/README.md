...
| 2 | **Pocket SDS** — 78-chemical library → filter by GHS hazard → open a chemical → pictograms, PPE, sections |
...
```sh
ALLOW_DEV_LOGIN=1 NEXT_PUBLIC_SDS_MODULE=1 NEXT_PUBLIC_AI_ASSIST=1 ANTHROPIC_API_KEY=sk-ant-... npm run dev
```
...
- Seed data loads on first visit, so the first SDS beat waits for the library to
  populate before filtering.
